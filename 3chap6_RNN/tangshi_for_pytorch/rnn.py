...
class RNN_model(nn.Module):
    def __init__(self, batch_sz ,vocab_len ,word_embedding,embedding_dim, lstm_hidden_dim,device='cuda'):
        ...
        # ???
        self.lstm=nn.LSTM(embedding_dim,lstm_hidden_dim,2,batch_first=True)
    ...
    def forward(self,sentence,is_test = False):
        ...
        # ???
        output, _ = self.lstm(batch_input, (torch.zeros(2, 1, self.lstm_dim).to(self.device), torch.zeros(2, 1, self.lstm_dim).to(self.device)))
