...
def run_training():
    ...
    rnn_model = rnn_lstm.RNN_model(batch_sz = BATCH_SIZE,vocab_len = len(word_to_int) + 1 ,word_embedding = word_embedding ,embedding_dim= 100, lstm_hidden_dim=128,device=device)
    rnn_model.to(device)
...
def to_word0(predict, vocabs):  # 预测的结果转化成汉字
...
def to_word(predict, vocabs, temperature=0.3):#温度越大越随机
...
def gen_poem(begin_word,temperature=0.2):
    ...
    while word != end_token  :
        ...
        output = rnn_model(input, is_test=True).to('cpu')
        
        word = to_word(output.data.tolist()[-1], vocabularies, temperature)
        poem += word
        ...
        if len(poem) > 40:
            break
    if len(poem) < 12:
        poem = gen_poem(begin_word,temperature+0.05)
    return poem
